...
while True:
    ...
    cpu_usage = psutil.cpu_percent(interval=1)   # <--- CPU utilization in percentage measured across the cluster
    cpu_card.data.usage = cpu_usage
    cpu_card.plot_data[-1] = [tick, cpu_usage]

    mem_usage = psutil.virtual_memory().percent
    mem_card.data.usage = mem_usage              # <--- available memory in a managed cluster
    mem_card.plot_data[-1] = [tick, mem_usage]

    cpu_count = psutil.cpu_count()               # <-- total number of available cpu cores in a managed cluster
